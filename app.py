...
USUARIOS = {
    "admin": {"senha": "tigrao123", "nome": "Administrador", "perfil": "admin"},
    "vendedor": {"senha": "123", "nome": "Vendedor", "perfil": "vendedor"},
}
...
def garantir_colunas():
    ...
    if "fornecedor" not in fornecedores.columns:
        fornecedores = pd.DataFrame(columns=["fornecedor", "telefone", "cidade"])
...
# PRODUTOS
    ...
    lista_fornecedores = fornecedores_df["fornecedor"].dropna().astype(str).tolist() if "fornecedor" in fornecedores_df.columns else []
    lista_fornecedores = sorted([f for f in lista_fornecedores if f.strip() != ""])
    ...
    if usuario_admin():
        with st.expander("Cadastrar produto manual"):
            ...
            fornecedor = st.selectbox("Fornecedor", lista_fornecedores) if lista_fornecedores else st.text_input("Fornecedor")
